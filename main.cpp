...
int main() {
  ...
  start = clock();
  heapSort(a, size_array); // вызов функции сортировки,вывод свапов и сравнений
  stop = clock();
  printf("\n");
  cout << "HeapSort Time Spend: " << (stop - start) / 1000.0 << "ms" << endl;

  cout << "Сортировка Шелла" << endl;
  start = clock();
  ShellSort(b, size_array);
  stop = clock();
  cout << "ShellSort Time Spend: " << (stop - start) / 1000.0 << "ms" << endl;
  return 0;
}
